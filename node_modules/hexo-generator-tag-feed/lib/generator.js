...
module.exports = function(locals) {
  ...
  return locals.tags.map(function (tag) {
    ...
    var xml = template.render({
      ...
      url: url,
      posts: posts,
      feed_url: config.root + feedConfig.path.pre + '/' + tag.name + '/' + feedConfig.path.post
    });
    return {
      path: feedConfig.path.pre + '/' + tag.name + '/' + feedConfig.path.post,
      data: xml
    };
  });
};
